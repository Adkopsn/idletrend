...
function ChartTooltip({
  ...
  const price = payload.find(
    (item) => item.dataKey === "fiyat"
  )?.value;

  const listings = payload.find(
    (item) => item.dataKey === "ilan"
  )?.value;
...
export default function ItemDetailPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/items/${id}`)
      .then((response) => {
        if (!response.ok) {
          throw new Error("Item detayı alınamadı.");
        }
      ...
      .catch((fetchError) => {
        console.error(
          "Item detayı alınamadı:",
          fetchError
        );

        setError("Item detayı yüklenemedi.");
        setLoading(false);
      });
  }, [id]);
  ...
  const priceChangePercent =
    previous && previous.priceCents > 0
      ? (priceChangeCents / previous.priceCents) * 100
      : 0;

  const chartData = [...item.history]
    .reverse()
    .map((record) => ({
      tarih: new Date(record.fetchedAt).toLocaleString(
        "tr-TR",
        {
          day: "2-digit",
          month: "2-digit",
      ...
      fiyat: record.priceCents / 100,
      ilan: record.listings,
    }));
            ...
            <div className="flex-1">
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-3">
                ...
                <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-4">
                  ...
                  <p className="mt-1 text-xl font-semibold">
                    {latest
                      ? latest.listings.toLocaleString("tr-TR")
                      : "-"}
                  </p>
                </div>
        ...
        <section className="mt-8 rounded-2xl border border-zinc-800 bg-zinc-900 p-4 sm:p-6">
          <div className="mb-6">
            <h2 className="text-xl font-semibold">
              Fiyat ve İlan Geçmişi
            </h2>

            <p className="mt-1 text-sm text-zinc-400">
              Steam Market snapshot kayıtları
            </p>
          </div>

          {chartData.length < 2 ? (
            <div className="flex h-72 items-center justify-center rounded-xl bg-zinc-950 text-zinc-400">
